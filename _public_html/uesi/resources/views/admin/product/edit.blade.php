...
@section('content')
    ...
    <div class="container-fluid">
      ...
      <form method="post" class="row" action="{{ route('product.update',['id' => $product->id]) }}" enctype="multipart/form-data" autocomplete="off">
          @csrf
          @method('post')
            ...
            $(`#${tableId}`).append(`<tr class="tr_attribute_clone">
                                        <td><select class="form-control select2" name="attributesArray[${counter_attribute}][attribute_id]">
                                                @foreach($data['attributeArray'] as $key => $value)
                                                    <optgroup label="{{  $value->ProductAttributeGroupDescription->name }}">
